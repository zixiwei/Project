...
def result_generator(directory_path, input_file, threshold):
    ...
    output_directory = os.path.join(directory_path, "$VIDEO_NAME - Scene $SCENE_NUMBER.mp4")
...
result_generator(path_scene, input_path, 63)
...
for filename in enumerate(os.listdir(path_scene)):
    if filename[1].endswith(".mp4"):
        ...
        if os.path.isfile(path_shot):
            ...
            result_generator(path_shot_result, path_shot, 27)
            for file_name in enumerate(os.listdir(path_shot_result)):
                if file_name[1].endswith(".mp4"):
                    ...
                    if os.path.isfile(path_sub_shot):
                        ...
                        result_generator(path_sub_shot_result, path_sub_shot, 14)
